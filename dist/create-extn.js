...
`, { encoding: 'utf-8' });
    fs.writeFileSync(`${templatesRoot}/package.json`, `{
...
async function createDevQuasarProject() {
    ...
    console.log(' \x1b[32mquasar-generate •\x1b[0m', 'Creating Quasar project for \x1b[47mdev\x1b[0m...');
    const answersMap = {
...
function fixCompileTimeYarnPnP() {
    ...
    extendJsonFile(devPackageJsonFilePath, packages.map((item) => ({
    ...
    fs.writeFileSync(`${extensionRoot}/.yarn/patches/vite-plugin-checker-npm-0.12.0-20cc09e28e.patch`, `diff --git a/dist/checkers/eslint/main.js b/dist/checkers/eslint/main.js
...
`, { encoding: 'utf-8' });
    ...
    // Commit code.
    execSync(`cd ${extensionRoot} && git add . && git commit -q -m "\\\`fixCompileTimeYarnPnP()\\\`"`, {
        stdio: 'inherit',
    });
}
// Workspaces formatting and linting
...
function rootWorkspaceFormattingAndLintingScrips() {
    ...
    extendJsonFile(extensionPackageJsonFilePath, [
        ...
        {
            path: 'scripts.format',
            value: 'prettier --write "**/*.{js,ts,vue,scss,html,md,json}" --ignore-path dev/.gitignore  --ignore-path .prettierignore',
        },
        {
            path: 'scripts.clean',
    ...
    extendJsonFile(extensionPackageJsonFilePath, [
        {
            ...
            value: 'patch:@quasar/app-vite@npm%3A2.5.2#~/.yarn/patches/@quasar-app-vite-npm-2.5.2-129f3acda8.patch',
        },
    ]);
    fs.mkdirSync(`${extensionRoot}/.yarn/patches`, { recursive: true });
    fs.writeFileSync(`${extensionRoot}/.yarn/patches/@quasar-app-vite-npm-2.5.2-129f3acda8.patch`, `diff --git a/package.json b/package.json
...
function devWorkspaceSrc() {
    ...
    fs.writeFileSync(`${devRoot}/.${config.extensionId.replace(/-/g, '')}rc.js`, `export default {
  modules: {},
};
`, { encoding: 'utf-8' });
    // Add workspace template.
...
function finishRootWorkspace() {
    ...
    extendJsonFile(extensionPackageJsonFilePath, [
        ...
        { path: 'scripts.watch', value: 'yarn tsc --watch' },
        {
            ...
            value: 'cd ./templates && node ./buildPaths.js && yarn prettier --write ./tsconfig-paths.json',
        },
    ]);
    // Commit code.
...
function finishAllAndLaunch() {
    ...
    if (runYarn) {
        execSync(`cd ${extensionRoot} && yarn && cd dev && yarn postinstall && cd .. && yarn buildPaths && yarn build && yarn clean && cd dev && yarn i-${config.extensionId} && yarn dev`, {
            stdio: 'inherit',
        });
    }
    else {
        console.log(`                   Run \x1b[47mcd ${extensionRoot} && yarn && cd dev && yarn postinstall && cd .. && yarn buildPaths && yarn build && yarn clean && cd dev && yarn i-${config.extensionId} && yarn dev\x1b[0m manually.`);
    }
    // Auto launch
    if (autoLaunch) {
