...
f ||
    fixCompileTimeYarnPnP(extensionRoot, extAppRoot, true, true, `\\\`fixCompileTimeYarnPnP()\\\` for \\\`${config.packageName}\\\``);
...
function projectSrc() {
    ...
    fs.writeFileSync(`${extAppRoot}/.${extensionId.replace(/-/g, '')}rc.js`, `export default {
  modules: {},
};
`, { encoding: 'utf-8' });
    // Add shared template.
    ...
    if (!(extensionOrganizationName === 'motinet' && extensionId === 'mnapp') &&
