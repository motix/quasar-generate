...
function templatesWorkspaceFormattingAndLinting() {
    ...
    extendJsonFile(templatesPackageJsonFilePath, [
        ...
        {
            path: 'scripts.format',
            value: 'prettier --write "**/*.{js,ts,vue,css,scss,html,md,json}" --ignore-path ../dev/.gitignore',
        },
        {
            path: 'scripts.clean',
            value: 'yarn format --log-level warn && yarn lint --fix',
...
function devWorkspaceSrc() {
    ...
    fs.writeFileSync(`${devWorkspaceFolder}/.${projectConfig.extensionId.replace(/-/g, '')}rc.js`, `export default {
  modules: {},
};
`, 'utf-8');
    // Apply project template.
    ...
    if (mnappDetected()) {
        extendJsonFile(devPackageJsonFilePath, [
            ...
            },
            {
                ...
                value: projectConfig.mnappLocation || packagesVersion['@motinet/quasar-app-extension-mnapp'],
            },
        ]);
    }
    // Commit code.
