...
async function createFirebaseWorkspace() {
    ...
    console.log(' \x1b[32mquasar-generate •\x1b[0m', `Initializing Firebase project for \x1b[47m${config.packageName}\x1b[0m...`);
...
`, { encoding: 'utf-8' });
    ...
    // Commit code.
    commitCodeEnabled &&
        commitCode(rootWorkspaceFolder, `\\\`firebaseFormattingAndLinting()\\\` for \\\`${config.packageName}\\\``);
}
function functionsFormattingAndLinting() {
    ...
    // Commit code.
    commitCodeEnabled &&
        commitCode(rootWorkspaceFolder, `\\\`functionsFormattingAndLinting()\\\` for \\\`${config.packageName}\\\``);
}
// Workspaces base source code
function firebaseWorkspaceSrc() {
    ...
    // Commit code.
    commitCodeEnabled &&
        commitCode(rootWorkspaceFolder, `\\\`firebaseWorkspaceSrc()\\\` for \\\`${config.packageName}\\\``);
}
...
`;
    ...
    // Commit code.
    commitCodeEnabled &&
        commitCode(rootWorkspaceFolder, `\\\`finishFirebaseWorkspace()\\\` for \\\`${config.packageName}\\\``);
}
function finishFunctionsWorkspace() {
    ...
    // Commit code.
    commitCodeEnabled &&
        commitCode(rootWorkspaceFolder, `\\\`finishFunctionsWorkspace()\\\` for \\\`${config.packageName}\\\``);
}
