...
export default function fixCompileTimeYarnPnP(options) {
    ...
    const rootPackageJsonFilePath = rootWorkspaceFolder === undefined ? undefined : `${rootWorkspaceFolder}/package.json`;
    ...
    rootPackageJsonFilePath !== undefined &&
        extendJsonFile(rootPackageJsonFilePath, [
            // Everytime editor SDKs related packages are added or removed, update editor SDKs
            {
                ...
                value: targetWorkspaceFolder === rootWorkspaceFolder // Standalone Quasar app
    ...
    if (rootWorkspaceFolder !== undefined &&
        ...
        const relativePathToYarn = path.join(path.relative(targetWorkspaceFolder, rootWorkspaceFolder), '.yarn');
        fs.writeFileSync(`${targetWorkspaceFolder}/vite.config.ts`, `import { homedir } from 'node:os';
...
`, { encoding: 'utf-8' });
    ...
    commitCodeMessage !== undefined &&
        ...
        rootWorkspaceFolder || targetWorkspaceFolder, commitCodeMessage || '\\`fixCompileTimeYarnPnP()\\`');
}
