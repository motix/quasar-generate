...
const validationSchema = markRaw(
  object({
    name: stringRequired('Name').test({
      ...
      test: async (value) =>
        !value ||
        (await validateUniqueField('production_productTypes', 'name', value, vm.value.id)),
    }),
...
const $p = useEditPage<ProductType<never>, ProductTypeVm>(props.scopeName);
const {
  // Auto sort
  dirty,
  vm,
...
<template>
  <ExpandableCard
    avatar-icon="fal fa-cards-blank"
    body-row-gutter
