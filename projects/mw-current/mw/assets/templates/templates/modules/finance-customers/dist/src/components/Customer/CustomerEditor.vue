...
const validationSchema = markRaw(
  object({
    code: stringRequired('Code').test({
      ...
      test: async (value) =>
        !value || (await validateUniqueField('finance_customers', 'code', value, vm.value.id)),
    }),
    name: stringRequired('Name').test({
      message: 'Name is already taken',
      test: async (value) =>
        !value || (await validateUniqueField('finance_customers', 'name', value, vm.value.id)),
    }),
  }),
);
...
const $p = useEditPage<Customer, CustomerVm>(props.scopeName);
const {
  // Auto sort
  dirty,
  vm,
