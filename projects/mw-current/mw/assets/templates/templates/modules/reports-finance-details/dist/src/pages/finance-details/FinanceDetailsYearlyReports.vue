...
import FinanceDetailsYearlyReportCharts from 'components/FinanceDetails/charts/FinanceDetailsYearlyReportCharts.vue';
...
const reportTitle = computed(() =>
  report.value ? `Finance Details Yearly Report ${report.value.year}` : '',
);
</script>
  ...
  <QPagePadding padding>
      ...
      <FadeTransition>
        <template v-if="!!report">
          <FadeTransition>
            ...
            <FinanceDetailsYearlyReportCharts
    ...
    <FloatToolbar>
      <template #fixed-buttons>
        ...
        <SwitchViewButton
          v-if="!!report && report.content.details.length > 0 && !isChartsView"
          key="switchView"
        />

        <ToggleChartsViewButton
          v-if="!!report && report.content.details.length > 0"
