...
import type { InvoiceGroup } from 'models/finance/index.js';
...
const SCOPE_NAME = 'invoice-groups-list-page';
...
const $p = useListPage<InvoiceGroup, InvoiceGroup>(SCOPE_NAME, true);
const {
  // Auto sort
  listItemCardWidth,
  onItemClick,
...
const { store, onLoadNextPage } = useListPageStore<InvoiceGroup>(
  useInvoiceGroupsStore(),
  $p.loadPage,
  $p.appendItems,
  $p.updateItems,
...
>(
  ...
  {
    type: 'active',
    label: 'Active Invoice Groups',
    queryConstraints: [where('isActive', '==', true), ...invoiceGroupsStoreDefaultSort],
  },
  {
    type: 'all',
    label: 'All Invoice Groups',
    queryConstraints: [...invoiceGroupsStoreDefaultSort],
  },
);
...
$p.viewUrl.value = '/invoice-groups/';

// useNavigateToNewPage
$p.newUrl.value = '/invoice-groups/new';
</script>
