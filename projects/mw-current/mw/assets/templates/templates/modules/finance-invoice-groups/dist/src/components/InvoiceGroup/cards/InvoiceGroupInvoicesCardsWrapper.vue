...
const {
  ...
  invoicesReady,
  onInvoiceClick,
} = useInvoiceGroupInvoicesList(props);
  ...
  <FadeTransition>
    ...
    <div v-else key="ready" class="q-gutter-y-md">
      ...
      <div class="row items-start justify-evenly q-gutter-md q-mt-none q-pb-xs">
        <ExpandableCard
          v-for="{ invoice } in invoiceRecords"
          :key="invoice.id"
          ...
          :external-link-url="`/general-invoices/${invoice.code.replaceAll('.', '_')}`"
          header-separator
          side-top
          ...
          :title="invoice.code"
          @click="invoice.code === currentInvoiceCode ? undefined : onInvoiceClick(invoice)"
        >
          <template #main>
