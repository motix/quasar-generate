...
import { membersStoreDefaultSort, useMembersStore } from 'stores/production/Members.js';
...
$p.columns.value = [
  ...
  {
    name: 'defaultProductionRole',
    label: 'Default Production Role',
    align: 'left',
    field: (row) => row.defaultProductionRole?.name,
  },
  {
    name: 'isActive',
    label: 'Active',
    ...
    <ListPage :composition="$p" :scope-name="SCOPE_NAME" @load-next-page="onLoadNextPage">
      ...
      <template #item-card="{ model, link }">
        <ExpandableCard
          ...
          :caption="model.defaultProductionRole?.name"
          clickable
