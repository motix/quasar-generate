...
const payableChartOptions = computed(
  () =>
    ({
      ...
      labels: props.report.map((value) => value.supplier.name),
    }) as ApexOptions,
);

const payableChartSeries = computed(() => [
  {
    name: 'Balance',
...
<template>
  <div>
    <h4 class="text-weight-light">Payable</h4>

    <Apex-Charts
      :height="chartHeight"
      :options="payableChartOptions"
      :series="payableChartSeries"
      type="bar"
    />
  </div>
</template>
