...
import { useTransactionViewPage_Expenses } from 'composables/finance/transaction/useTransactionEditPage_Expenses.js';
...
import TransactionViewer from 'components/Transaction/TransactionViewer_Expenses.vue';

// Constants

const SCOPE_NAME = 'general-expense-transactions-view-page';
...
useTransactionViewPage<true, Expense, ExpenseVm>(SCOPE_NAME, true, true);
const $p = useTransactionViewPage_Expenses<true>(SCOPE_NAME);
const {
  // Auto sort
  addDetail,
  childViewerRef,
...
// useReturnUrl
$p.backUrl.value = '/general-expense-transactions';
...
$p.selectNextChildAfterRemoving.value = (children) =>
  ...
  (() => {
    throw new Error('[finance-expenses] - Index out of range');
  })();
...
// useViewPageExpenseExtra
$p.fillDetailsFromParent.value = $p.fillDetailsFromExpense;
...
watch(
  ...
  (value) => {
    if (value === 0) {
      delete meta.history;
      $p.backUrl.value = `/general-expenses/${$p.parentModel.value?.code.replaceAll('.', '_')}`;
    }
  },
);
</script>
