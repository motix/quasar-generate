...
import useExpenseDetailEditor from 'composables/finance/expense/useExpenseDetailEditor.js';
...
const $p = useExpenseDetailEditor(props);
const {
  // Auto sort
  detail,
  dirty,
...
defineExpose({
  validate: $p.validateExpenseDetailEditor,
});
</script>
  ...
  >
    ...
    <q-card-section class="q-col-gutter-md row">
      ...
      <TextField class="col-12" dense field-class="text-right" label="Amount" stack-label>
        {{ f.currency(vmc.expenseDetailAmount(detail)) }}
      </TextField>
    </q-card-section>
  </q-card>
</template>
