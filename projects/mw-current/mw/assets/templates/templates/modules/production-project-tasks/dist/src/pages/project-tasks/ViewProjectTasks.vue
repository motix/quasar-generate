...
const SCOPE_NAME = 'project-tasks-view-page';
...
const $p = useTasksViewPage<Project, ProjectVm>(SCOPE_NAME, true);
const {
  // Auto sort
  copyButtonEnabled,
  copySelectedNode,
...
$p.toolbarExtraButtonVisibility.value.copy = hasRole('project-leader');
$p.toolbarExtraButtonVisibility.value.cut = hasRole('project-leader');
$p.toolbarExtraButtonVisibility.value.paste = hasRole('project-leader');

// useTree
$p.hasNodeDeleting.value = hasRole('project-leader');

// useTasksViewPage
$p.routePathRoot.value = '/project-tasks';
$p.storagePathRoot.value = 'production/project-tasks';
...
watch($p.selectedNodeKey, (value) => {
  $p.hasEditor.value = !!value && hasRole('project-leader');
});
...
watchEffect(() => {
  ...
  $p.toolbarExtraButtonVisibility.value.copy =
    hasRole('project-leader') && !$p.filtersApplied.value && !$p.editMode.value;
  $p.toolbarExtraButtonVisibility.value.cut =
    hasRole('project-leader') && !$p.filtersApplied.value && !$p.editMode.value;
  $p.toolbarExtraButtonVisibility.value.paste =
    hasRole('project-leader') && !$p.filtersApplied.value && !$p.editMode.value;
});
</script>
    ...
    <ViewPage :scope-name="SCOPE_NAME">
      <template #viewer>
        <div class="q-gutter-y-lg">
          <ProjectViewerMain expandable readonly :scope-name="SCOPE_NAME" />
