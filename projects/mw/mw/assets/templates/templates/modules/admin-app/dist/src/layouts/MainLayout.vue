...
<template>
  <q-layout view="hHh lpr fFf" @scroll="floatToolbarOnScroll">
    <q-header
    ...
    <q-footer class="transparent">
      <FadeTransition>
        <q-tabs v-if="hasRole('maintenance')" class="bg-grey-8 text-white shadow-up-5" dense>
          <q-route-tab label="Home" name="home" to="/" />

          <!-- <% if (config.hasModule('admin-membership')) { %>•+ admin-membership -->
          <q-route-tab label="Team" name="team" to="/team" />
          <q-route-tab label="Accounts" name="accounts" to="/accounts" />
          <!-- •- /admin-membership<% } else { %>•! admin-membership absent<% } %> -->

          <!-- <% if (config.hasModule('admin-slack')) { %>•+ admin-slack -->
          <q-route-tab label="Slack" name="slack" to="/slack" />
          <!-- •- /admin-slack<% } else { %>•! admin-slack absent<% } %> -->

          <!-- <% if (config.hasModule('admin-health')) { %>•+ admin-health -->
          <q-route-tab v-if="hasRole('admin')" label="Health" name="health" to="/health" />
          <!-- •- /admin-health<% } else { %>•! admin-health absent<% } %> -->

          <q-route-tab :href="portalUrl" label="Portal" name="portal" />
        </q-tabs>
        <q-tabs v-else dense>
