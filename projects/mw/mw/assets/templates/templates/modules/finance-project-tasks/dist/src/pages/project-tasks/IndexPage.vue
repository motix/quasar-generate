    ...
    <ListPage :composition="$p" :scope-name="SCOPE_NAME" @load-next-page="onLoadNextPage">
      ...
      <template #item-card="{ model, link }">
        <ExpandableCard
          ...
          :subtitle="f.date(c(model).finishDate) || undefined"
        ...
        >
          <template #main>
            <StatusBadge class="q-mt-sm" :status="c(model).statusHelper" />
          </template>

          <template #side>
            <q-toggle
              v-model="c(model).isArchived"
              checked-icon="fal fa-box-taped"
              class="right-toggle"
              color="primary"
              disable
          ...
          <template
            v-if="
              mc.allAssignedTo(c(model).tasks).length > 0 ||
              mc.totalNew(c(model).tasks) > 0 ||
              mc.totalImplemented(c(model).tasks) > 0 ||
              mc.totalTested(c(model).tasks) > 0 ||
              mc.totalClosed(c(model).tasks) > 0
            "
            #body
          >
            <div class="col-12 text-caption">
              <TaskAssignedTo icon :members="mc.allAssignedTo(c(model).tasks)" size="sm" />
            </div>

            <div class="col-3 text-cyan text-caption">
              <span v-if="mc.totalNew(c(model).tasks) > 0">
                <q-icon name="fal fa-bars-progress" />
                {{ mc.totalNew(c(model).tasks) }}
                <TopTooltip>New</TopTooltip>
              </span>
            </div>
            <div class="col-3 text-primary text-caption">
              <span v-if="mc.totalImplemented(c(model).tasks) > 0">
                <q-icon name="fal fa-check-double" />
                {{ mc.totalImplemented(c(model).tasks) }}
                <TopTooltip>Implemented</TopTooltip>
              </span>
            </div>
            <div class="col-3 text-green text-caption">
              <span v-if="mc.totalTested(c(model).tasks) > 0">
                <q-icon name="fal fa-thumbs-up" />
                {{ mc.totalTested(c(model).tasks) }}
                <TopTooltip>Tested</TopTooltip>
              </span>
            </div>
            <div class="col-3 text-grey text-caption">
              <span v-if="mc.totalClosed(c(model).tasks) > 0">
                <q-icon name="fal fa-circle-xmark" />
                {{ mc.totalClosed(c(model).tasks) }}
