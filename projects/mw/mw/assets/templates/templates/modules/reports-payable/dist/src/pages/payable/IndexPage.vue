...
import { usePayableReportStore } from 'stores/reports/PayableReport.js';

import useCharts from 'composables/useCharts.js';
import useMultiViews from 'composables/useMultiViews.js';
import useNotifications from 'composables/useNotifications.js';

import PayableReportCards from 'components/Payable/cards/PayableReportCards.vue';
import PayableReportChart from 'components/Payable/charts/PayableReportChart.vue';
import PayableReportTable from 'components/Payable/table/PayableReportTable.vue';
...
function usePageData(ready: ReturnType<typeof usePageStatus>['ready']) {
  ...
  const store = usePayableReportStore();
  ...
  function loadReport() {
    ...
    store
      ...
      .then(() => {
        ...
        report.value = store.report as PayableReport;
        ready.value = true;
      })
      .catch((error) => {
        console.error(error);
  ...
  function refresh() {
    store.releasePayableReport({ immediately: true });
    loadReport();
  }
  ...
  onUnmounted(() => {
    store.releasePayableReport({ immediately: false });
  });
      ...
      <div v-else key="ready">
        ...
        <FadeTransition>
          ...
          <PayableReportCards
            ...
            :report="report"
          />
          <PayableReportChart v-else-if="isChartsView" key="chartsView" :report="report" />
        </FadeTransition>
      </div>
    </FadeTransition>
