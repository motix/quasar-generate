...
import useTransactionEditPage_Invoices from 'composables/finance/transaction/useTransactionEditPage_Invoices.js';
...
const $p = useTransactionEditPage_Invoices<true>(props.scopeName);
const {
  // Auto sort
  editorReady,
  f,
      ...
      <q-card-section class="q-col-gutter-sm row">
        ...
        <div class="col-6">
          <ObjectLink
            color="primary"
            :label="pvm.code"
            :to="`/general-invoices/${pvm.code.replaceAll('.', '_')}`"
          >
            <template #icon>
              <StatusIcon
                class="q-mr-sm"
                icon="fal fa-file-invoice-dollar"
                :status="pvm.statusHelper"
              />
            </template>
          </ObjectLink>
        </div>
