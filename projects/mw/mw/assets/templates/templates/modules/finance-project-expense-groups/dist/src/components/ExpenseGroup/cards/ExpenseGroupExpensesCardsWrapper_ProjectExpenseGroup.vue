...
import useExpenseGroupExpensesList_ProjectExpenseGroup from 'composables/finance/expense-group/useExpenseGroupExpensesList_ProjectExpenseGroup.js';
...
const {
  ...
  f,
  onExpenseClick,
} = useExpenseGroupExpensesList_ProjectExpenseGroup(props);
  ...
  <FadeTransition>
    ...
    <div v-else key="ready" class="q-gutter-y-md">
      ...
      <div class="row items-start justify-evenly q-gutter-md q-mt-none q-pb-xs">
        <ExpandableCard
          v-for="{ expense, parent: project } in expenseRecords"
          :key="expense.id"
          ...
          :external-link-url="
            project
              ? buidHasParentExpenseLink!(expense, project)
          ...
          :title="expense.code"
          @click="
            expense.code === currentExpenseCode ? undefined : onExpenseClick(expense, project)
          "
        >
          <template #main>
