...
import { changeGeneralExpenseTransactionStatus } from 'services/finance/expenses.js';
...
const $p = useTransactionViewPage_Expenses<true>(props.scopeName);
const {
  // Auto sort
  f,
  freezed,
...
async function changeStatus(action: TransactionActionName) {
  ...
  try {
    await changeGeneralExpenseTransactionStatus($p.pm.value.id, $p.m.value.code, action);
  } catch (error) {
    console.error(error);
    notifySaveDataError();
    ...
    >
      ...
      <template #body>
        <div class="col-12 row justify-between">
          ...
          <div>
            <ObjectLink
              v-if="mc.transactionHasSupplier(m)"
              color="primary"
              icon="fal fa-building"
              :label="pm.supplier.name"
              :to="`/suppliers/${pm.supplier.code}`"
            />
            <ObjectLink
              v-if="
                mc.transactionHasSourceAccount(m) &&
        ...
        <div class="col-6">
          <ObjectLink
            color="primary"
            :label="pm.code"
            :to="`/general-expenses/${pm.code.replaceAll('.', '_')}`"
          >
            <template #icon>
              <StatusIcon class="q-mr-sm" icon="fal fa-receipt" :status="pm.statusHelper" />
            </template>
          </ObjectLink>
        </div>
