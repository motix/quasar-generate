...
import type { FinanceAccountLite, InvoiceGroupLite } from 'models/finance';

import { useInvoicesReportsStore } from 'stores/reports/InvoicesReports.js';

import useExportInvoicesRangeReportToExcel from 'composables/reports/invoice/useExportInvoicesRangeReportToExcel.js';
...
import InvoicesReportCards from 'components/Invoice/cards/InvoicesReportCards.vue';
import InvoicesReportTable from 'components/Invoice/table/InvoicesReportTable.vue';
...
function useFilters() {
  ...
  const showProjectInvoices = ref(true);
  const showGeneralInvoices = ref(true);
  const showAllFinanceAccounts = ref(false);
  const selectedFinanceAccounts = ref<{ [index: string]: boolean }>({});
  const showAllInvoiceGroups = ref(false);
  const selectedInvoiceGroups = ref<{ [index: string]: boolean }>({});

  // Methods

  function resetFilters() {
    showProjectInvoices.value = true;
    showGeneralInvoices.value = true;
    selectedFinanceAccounts.value = {};
    selectedInvoiceGroups.value = {};
  }

  function reloadFilters(financeAccounts: FinanceAccountLite[], invoiceGroups: InvoiceGroupLite[]) {
    ...
    selectedInvoiceGroups.value = invoiceGroups
      .map((value) => value.id)
      .reduce(
        (previousValue, currentValue) => ({
          ...previousValue,
  ...
  watch(showAllInvoiceGroups, (value) => {
    selectedInvoiceGroups.value = Object.keys(selectedInvoiceGroups.value).reduce(
      (previousValue, currentValue) => ({
        ...previousValue,
        [currentValue]: value,
      }),
  ...
  watchEffect(() => {
    if (
      Object.keys(selectedInvoiceGroups.value).length > 0 &&
      !Object.keys(selectedInvoiceGroups.value).some(
        (key) => selectedInvoiceGroups.value[key] === false,
      )
    ) {
      showAllInvoiceGroups.value = true;
    }

    if (
      Object.keys(selectedInvoiceGroups.value).length > 0 &&
      !Object.keys(selectedInvoiceGroups.value).some(
        (key) => selectedInvoiceGroups.value[key] === true,
      )
    ) {
      showAllInvoiceGroups.value = false;
    }
  });
...
function usePageData(
  ...
  const store = useInvoicesReportsStore();
  ...
  async function loadReport() {
    ...
    reloadFilters(
      ...
      report.value?.content.invoiceGroups || [],
    );
  }

  function clearCache() {
    store.releaseInvoicesReports({ immediately: true });
    report.value = null;
  }
  ...
  onUnmounted(() => {
    store.releaseInvoicesReports({ immediately: false });
  });
...
const { exportInvoicesRangeReportToExcel } = useExportInvoicesRangeReportToExcel();

// Computed

const reportTitle = computed(() =>
  report.value
    ? `Invoices Report ${f.date(report.value.startDate)} - ${f.date(report.value.endDate)}`
    : '',
);
...
<template>
  <QPagePadding padding>
    <div class="q-gutter-y-lg">
      ...
      <q-slide-transition>
        <q-card
          v-if="report?.content.invoiceGroups.length || 0 > 0"
          class="q-mx-auto"
          style="width: 100%"
          :style="{ maxWidth: cardWidth * 2 + 'px' }"
        >
          <q-card-section>
            ...
            <div class="q-col-gutter-md row justify-left">
              ...
              />
              <q-toggle
                v-for="group in report!.content.invoiceGroups"
                :key="group.id"
                v-model="selectedInvoiceGroups[group.id]"
                checked-icon="fal fa-check"
                class="col-6 col-sm-4 col-md-3"
                :label="group.name"
                unchecked-icon="clear"
        ...
        <template v-if="!!report">
          <FadeTransition>
            ...
            <InvoicesReportTable
              ...
              :selected-invoice-groups="selectedInvoiceGroups"
              :show-general-invoices="showGeneralInvoices"
              :show-project-invoices="showProjectInvoices"
            />
    ...
    <FloatToolbar>
      <template #fixed-buttons>
        ...
        <q-btn
          ...
          padding="sm"
          text-color="primary"
          @click="() => void exportInvoicesRangeReportToExcel(report!)"
        >
