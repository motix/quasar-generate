...
// finance_generalInvoices transactions finance accounts synced main
export const generalInvoicesTfasSyncedMain = onCallWithPermission(['admin'], () => {
  let itemCount = 0;

  return liteDocsSyncedMainCollection<Invoice, FinanceAccount, 'transactions'>(
    ...
    [],
    {
      ...
      successMessage: () =>
        `\`sourceFinanceAccount\` and \`destinationFinanceAccount\` of all ${itemCount} items(s) in \`finance_generalInvoices/{id}.transactions\` have the same info as in \`finance_financeAccounts\`.`,
    },
  );
});
