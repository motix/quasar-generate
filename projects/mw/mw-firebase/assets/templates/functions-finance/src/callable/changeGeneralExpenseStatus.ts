...
export const changeGeneralExpenseStatus = onCallWithPermission<
  { id: string; action: string },
  Promise<void>
>(['admin', 'finance'], async ({ data: { id, action }, auth }) => {
  const db = getFirestore();
  const docRef = db.doc(`finance_generalExpenses/${id}`) as DocumentReference<Expense, Expense>;
  ...
  await extraLog(
    ...
    `Changing status of general expense "${doc.code}" (${doc.content}) by action "${action}"...`,
  );
