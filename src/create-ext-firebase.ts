...
function firebaseWorkspaceSrc() {
  ...
  fs.copyFileSync(
    `${globalAssets}/rebuildFunctions.js`,
    `${firebaseWorkspaceFolder}/rebuildFunctions.js`,
  );

  let rebuildFunctionsJs = fs.readFileSync(
    `${firebaseWorkspaceFolder}/rebuildFunctions.js`,
    'utf-8',
  );
...
function functionsWorkspaceSrc() {
  ...
  fs.copyFileSync(
    `${globalAssets}/functions/refTools.js`,
    `${functionsWorkspaceFolder}/refTools.js`,
  );
  fs.copyFileSync(
    `${globalAssets}/functions/refUpdate.js`,
    `${functionsWorkspaceFolder}/refUpdate.js`,
  );
...
function installAndLaunch() {
  ...
  if (runYarn) {
    execSync(
      `cd ${firebaseWorkspaceFolder.replaceAll(' ', '\\ ')} && yarn && yarn rebuildFunctions && yarn clean && yarn serve`,
      {
        stdio: 'inherit',
      },
    );
